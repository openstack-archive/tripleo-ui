...
import { getCurrentStack } from './selectors/plans';
...
TempStorage.initialized.then(() => {
  ...
  function checkRunningDeployment(nextState, replaceState) {
    ...
    let currentStack = getCurrentStack(state);
    let currentPlanName = state.plans.currentPlanName;
    if(currentStack && currentStack.stack_name === currentPlanName &&
       currentStack.stack_status === 'CREATE_IN_PROGRESS') {
