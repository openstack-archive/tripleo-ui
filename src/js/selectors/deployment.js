      ...
      switch (status) {
        case 'RUNNING':
          if (type === MistralConstants.DEPLOYMENT_DEPLOY_PLAN) {
            ...
            return { status: 'UNDEPLOYING', message };
          }
        case 'FAILED':
        case 'ERROR':
          if (type === MistralConstants.DEPLOYMENT_DEPLOY_PLAN) {
            return { status: 'DEPLOYMENT_FAILED', message };
          } else if (type === MistralConstants.UNDEPLOY_PLAN) {
            return { status: 'UNDEPLOY_FAILED', message };
          }
        case 'SUCCESS':
          if (type === MistralConstants.DEPLOYMENT_DEPLOY_PLAN) {
            return { status: 'DEPLOYED', message };
          } else if (type === MistralConstants.UNDEPLOY_PLAN) {
            return { status: 'UNDEPLOYED', message };
          }
        default:
