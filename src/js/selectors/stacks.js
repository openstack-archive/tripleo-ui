...
/**
  ...
  * an empty Map if one of the values hasn't been fetched yet.
  */
export const getOvercloudInfo = createSelector(
  ...
  (currentStackEnvironment, currentPlanName, stackResourceDetails) => {
    ...
    const ipAddress = stackResourceDetails.get('PublicVirtualIP')
      ? stackResourceDetails.getIn([
          'PublicVirtualIP',
          'attributes',
          'ip_address'
        ])
      : null;
    const overcloudInfo = adminPassword && ipAddress
      ? { ipAddress: ipAddress, adminPassword: adminPassword }
      : {};
    return Map(overcloudInfo);
  }
);
