    ...
    nodes.map(node =>
      node
        ...
        .update(node =>
          node.set(
            'macs',
            node.get('portsDetail').reduce((str, v) => str + v.address, '')
          )
        )
    )
);
...
export const getFilteredRegisteredNodes = createSelector(
  ...
  (nodes, nodesToolbarFilter) =>
    nodes
      ...
      .update(
        nodes =>
          (nodesToolbarFilter.get('sortDir') === 'desc'
            ? nodes.reverse()
            : nodes)
      )
);
 ...
 * @returns function
 */
const filterPorts = ports => portUUIDs =>
  ports.filter((p, k) => portUUIDs.includes(k));
