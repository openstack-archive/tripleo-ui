  ...
  addNodesFromInstackenvJSON(fileContents) {
    ...
    try {
      ...
      this.setState({
        jsonErrors: [
          {
            title: this.props.intl.formatMessage(messages.invalidJson),
            message: e.toString()
          }
        ]
      });
        ...
        if (file.name.match(/(\.json)$/)) {
          ...
          this.props.notify({
            title: this.props.intl.formatMessage(
              messages.unsupportedFileFormat
            ),
            message: this.props.intl.formatMessage(
    ...
    return (
      <Tab key={index} isActive={node.uuid === this.props.selectedNodeId}>
        <a className="link" onClick={this.selectNode.bind(this, node.uuid)}>
          ...
          <span
            className="tab-action fa fa-trash-o"
...
function mapStateToProps(state) {
  return {
    canSubmit: allNodesToRegisterAreValid(state) &&
      !state.registerNodes.get('isRegistering'),
