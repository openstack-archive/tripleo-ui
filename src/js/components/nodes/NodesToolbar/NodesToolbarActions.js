...
class NodesToolbarActions extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      <FormGroup title={disabled ? intl.formatMessage(messages.disabledButtonsWarning) : ''}>
        <Button
        ...
        <ConfirmationModal
          ...
          question={this.props.intl.formatMessage(messages.deleteNodesModalMessage)}
...
const mapDispatchToProps = dispatch => ({
  setSubmitAction: action => dispatch(change('nodesListForm', 'submitAction', action)),
  setTag: tag => dispatch(change('nodesListForm', 'tag', tag)),
  submitForm: () => dispatch(submit('nodesListForm'))
});

export default injectIntl(connect(mapStateToProps, mapDispatchToProps)(NodesToolbarActions));
