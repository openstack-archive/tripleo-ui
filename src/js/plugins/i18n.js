...
I18nPlugin.prototype.apply = function(compiler) {
  ...
  var locales = files.map(function(file) { return file.replace('.json', '');});
  ...
  var messagesImport = localesSimple.map(function(locale, index) {
    var lower = localesLower[index];
    var full = locales[index];
    return 'import ' + lower + 'Messages from \'../../../../i18n/locales/' +
      full + '.json\';';
  }).join('\n');

  var reactIntlImport = localesSimple.map(function(locale, index) {
    return 'import ' + locale + ' from \'react-intl/locale-data/' + locale + '\';';

  }).join('\n');

  var messages = locales.map(function(locale, index) {
    var lower = localesLower[index];
  ...
  var localeData = localesSimple.map(function(locale) {
    return '...' + locale;
  }).join(', ');

  var localeDataObj = 'export const LOCALE_DATA = [' + localeData + '];';

  var file = messagesImport + '\n\n' + reactIntlImport + '\n\n' + messagesObj +
      '\n\n'+ localeDataObj;
