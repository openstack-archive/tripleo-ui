...
import { Network, NetworksState } from '../immutableRecords/networks';

const initialState = new NetworksState();
...
const networksReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case NetworksConstants.FETCH_NETWORKS_SUCCESS: {
      const networks = fromJS(action.payload).map(network =>
        new Network(network).update(network =>
          network.set('identifier', network.name)
        )
      );
