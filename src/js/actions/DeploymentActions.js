...
const messages = defineMessages({
  deploymentFailedNotificationTitle: {
    id: 'DeploymentActions.deploymentFailedNotificationTitle',
  ...
  if (state === 'ERROR') {
    dispatch(
      NotificationActions.notify({
        ...
        // NOTE(jtomasek): this is ugly, message should always be a string, no check shoul be needed
        message: typeof message === 'string' ? message : JSON.stringify(message)
      })
    );
    dispatch(deploymentFailed(planName, message));
  } else {
    dispatch(deploymentSuccess(planName));
  }
};
